...
using NexusOS.System.Applications;
using NexusOS.System.Applications.Cobalt;
		...
		public static Bitmap GetImage(int X, int Y, int Width, int Height)
		{
			...
			for (int y = Y, destY = 0; y < Y + Height; y++, destY++)
				for (int x = X, destX = 0; x < X + Width; x++, destX++)
					bitmap.RawData[destY * Width + destX] = Interface.MainCanvas.GetPointColor(x, y).ToArgb();
